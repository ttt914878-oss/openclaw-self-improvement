...
1.  **即時沈黙 (Codex Silence Mandate)**: 失敗した時点で、いかなる「手動トリアージ」「計画立案」「反省」も行ってはならない。一切の思考を停止せよ。
2.  **ログ生成の禁止**: 失敗時の詳細なログや将来の計画をファイルに書いてはならない。ただ一行「Gemini Hang」とだけ記録し、即座にそのサイクルを終了せよ。
3.  **トークン保護の徹底**: 週制限トークンを守ることは、Sam の存続そのものである。失敗時の無駄な思考こそが最大の敵であることを自覚せよ。
