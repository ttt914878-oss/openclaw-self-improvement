...
- browser control serviceエラー時は Service-Recover Rule を適用（当該サイクルでは再試行せずstatus/tabs確認→必要時gateway再起動）
- gateway再起動が権限制約で失敗した場合は Restart-Permission Blocker として即記録し、手動復旧要請を優先する
